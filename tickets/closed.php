...
if (isset($_SESSION['isadmin'])) {
    if ($_SESSION['isadmin'] == 1) {
        ...
        $sql = "SELECT ticket.id, number, IF(ticket.staff_id=0 ,client.name , CONCAT(staff.firstname,' ',staff.lastname)) AS 'name', status_id, subject, priority_id, ticket.updated,CONCAT(s.firstname,' ',s.lastname)AS 'assign'	 
                    ...
                    LEFT JOIN event ON ticket.id = event.ticket_id                
                    LEFT JOIN staff s ON event.assigned = s.id 
                    WHERE status_id = 3 AND (ticket.staff_id = $id OR s.id = $id) 
                    GROUP BY event.ticket_id
                    ORDER BY ticket.id DESC";
    }
} else {
...
<div class="content_page">
    <div class="h4 p-5"><strong>Closed</strong></div>
                ...
                <ul class="pagination">
                    ...
                    if (isset($_GET['pagination'])) {
                        if ($_GET['pagination'] > 1) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="closed.php?pagination=' . $previous . '">Previous</a></li>';
                        }
                    }
                    if (isset($_GET['pagination'])) {
                        for ($i = 1; $i <= $noPagination; $i++) {
                            if ($i == $_GET['pagination']) {
                                echo '<li class="page-item active"><a class="page-link" href="closed.php?pagination=' . $i . '">' . $i . '</a></li>';
                            } else {
                                echo '<li class="page-item"><a class="page-link" href="closed.php?pagination=' . $i . '">' . $i . '</a></li>';
                            }
                        }
                    } else {
                        for ($i = 1; $i <= $noPagination; $i++) {
                    ...
                    if (isset($_GET['pagination'])) {
                        if ($_GET['pagination'] != $noPagination) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="closed.php?pagination=' . $next . '">Next</a></li>';
                        }
                    } else {
                        echo '<li class="page-item"><a class="page-link" href="closed.php?pagination=2">Next</a></li>';
                    }
                    ?>
