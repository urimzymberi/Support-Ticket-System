...
if (isset($_SESSION['isadmin'])) {
    if ($_SESSION['isadmin'] == 1) {
        ...
        $sql = "SELECT t.id, number, IF(t.staff_id=0 ,client.name , CONCAT(staff.firstname,' ',staff.lastname)) AS 'name', status_id, subject, priority_id, t.updated,CONCAT(s.firstname,' ',s.lastname)AS 'assign'	 
                    FROM ticket t
                    LEFT join client on t.client_id = client.id
                    LEFT JOIN staff ON t.staff_id = staff.id
                    LEFT JOIN event ON t.id = event.ticket_id                  
                    LEFT JOIN staff s ON event.assigned = s.id  
                    WHERE answered = 1 AND (t.staff_id = $id OR s.id = $id) 
                    GROUP BY event.ticket_id
                    ORDER BY t.id DESC";
    }
} else {
    $sql = "SELECT ticket.id, number, IF(ticket.staff_id=0 ,client.name , CONCAT(staff.firstname,' ',staff.lastname)) AS 'name', status_id, subject, priority_id, ticket.updated	 
                    ...
                    LEFT JOIN staff ON ticket.staff_id = staff.id
                    LEFT JOIN event ON ticket.id = event.ticket_id 
                    WHERE answered = 1 AND ticket.client_id = $id 
                    GROUP BY event.ticket_id 
                    ORDER BY ticket.id DESC";
}
...
<div class="content_page">
    <div class="h4 p-5"><strong>Answered</strong></div>
                ...
                <ul class="pagination">
                    ...
                    if (isset($_GET['pagination'])) {
                        if ($_GET['pagination'] > 1) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="answered.php?pagination=' . $previous . '">Previous</a></li>';
                        }
                    }
                    if (isset($_GET['pagination'])) {
                        for ($i = 1; $i <= $noPagination; $i++) {
                            if ($i == $_GET['pagination']) {
                                echo '<li class="page-item active"><a class="page-link" href="answered.php?pagination=' . $i . '">' . $i . '</a></li>';
                            } else {
                                echo '<li class="page-item"><a class="page-link" href="answered.php?pagination=' . $i . '">' . $i . '</a></li>';
                            }
                        }
                    } else {
                        for ($i = 1; $i <= $noPagination; $i++) {
                    ...
                    if (isset($_GET['pagination'])) {
                        if ($_GET['pagination'] != $noPagination) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="answered.php?pagination=' . $next . '">Next</a></li>';
                        }
                    } else {
                        echo '<li class="page-item"><a class="page-link" href="answered.php?pagination=2">Next</a></li>';
                    }
                    ?>
